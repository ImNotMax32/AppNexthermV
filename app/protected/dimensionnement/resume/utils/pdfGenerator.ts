...
const determineImagePath = (data: GeneratePdfData): string => {
  ...
  // Si aérothermie est sélectionnée, retourner l'image RoPAck.png
  if (type_pac === 'Aérothermie') {
    console.log("Aérothermie détectée, utilisation de l'image RoPAck.png");
    return '/assets/img/RoPAck.png';
  }
...
const getProductImagePath = (productName: string): string => {
  // Vérifier d'abord si c'est un produit d'aérothermie
  const typePac = localStorage.getItem('type_pac');
  if (typePac === 'Aérothermie') {
    return '/assets/img/ROPACK_12_15-01.png'; // Image pour les produits aérothermie (chemin corrigé)
  }
  
  // Pour les autres produits, logique existante
...
if (resultDeperdition) {
  ...
  doc.text(`${resultDeperdition} kW`, box1X + boxWidth * 1.2 / 2, textY, { align: 'center' });
    ...
    const chauffageInfos = [
      { label: "Déperdition de la maison :", value: `${Math.round(parseFloat(data.building.heatLoss || '0') * 1000)} W` },
    ...
    const textY = newBoxY + (boxHeight / 3) + (textHeight / 4);
    doc.text(`${Math.round(parseFloat(deperditionText || '0') * 1000)} W`, box1X + boxWidth * 1.2 / 2, textY, { align: 'center' });
    ...
    doc.setFontSize(23);
    doc.setTextColor(81, 83, 74);
    doc.text("Données nécessaires pour la prime CEE", pageCenter, 175, { align: 'center' });
    ...
    addTableItem(col1X, currentY, "1 - Tbase en °C :", 
        "(La température extérieure minimale régionale qui est en dessous de 0°C partout en France)", 
        `${temperatureExterieure}°C`);
    currentY += lineHeight * 2;

    addTableItem(col1X, currentY, "2 - Déperditions thermiques à Tbase en W :", 
        "(La déperdition thermique totale de votre logement à la température extérieure minimale régionale)", 
    ...
    // Deuxième colonne
    currentY = startY;
    const puissancePac = data.selectedProduct?.selectedModel?.puissance_calo 
    ...
    // Point 8
    currentY += addTableItem(col2X, currentY, "8 - Puissance de la PAC à 140% des déperditions :",
    ...
    // Point 9
    currentY += addTableItem(col2X, currentY, "9 - Puissance de la PAC en (W) :",
    ...
    // Point 10
    currentY += addTableItem(col2X, currentY, "10 - Température d'eau pour la puissance de la PAC :",
...
console.log("Chemin de l'image maison:", housePath);
  ...
  if (data.selectedProduct.selectedModel) {
    ...
    doc.setFontSize(12);
    addTextSafely(doc,
      `De ${Math.round(parseFloat(String(model.puissance_calo || '0')) * 1000)} W et un COP jusqu'à ${String(model.cop || '')}`,
      101, 125
    );
    addTextSafely(doc,
      `ETAS jusqu'à ${String(model.etas || '')}%`,
      101, 130
    );
  }
