...
export async function POST(req: Request) {
  try {
    ...
    const { message, systemPrompt, conversationHistory, productId, powerOption } = await req.json() as { 
      message: string;
      systemPrompt?: string;
    ...
    let contextualPrompt = systemPrompt || `Tu es l'assistant virtuel de Nextherm, une entreprise spécialisée dans les solutions de chauffage écologiques, notamment la géothermie et l'aérothermie.
...
`;
        ...
        if (product.GuideInstallationURL) {
          const installationGuide = await loadInstallationGuide(product.GuideInstallationURL);
    ...
    // Créer la requête avec l'historique
